...
public class Sorters2120 {
	...
	//overloaded bubbleSort
	public static <T> void bubbleSort(List<T> theList, Comparator<T> comp){
		int lastToConsider = theList.size();
        while (lastToConsider > 1) {
            for (int j=0; j<lastToConsider-1; j++) {
                if (comp.compare(theList.get(j),theList.get(j+1)) >  0 ) {
                    swap2(theList,j,j+1);
                }
            }
            lastToConsider--;
	...
	//overloaded swap (Types must match)
	private static <T> void swap2(List<T> theList, int i1, int i2) {
	...
	**/
	...
	//overloaded selection
	 public static <T> void selectionSort(List<T> theList, Comparator<T> comp) {
		...
		for(int i=0;i<theList.size()-1;i++){
			...
			for(int j=i+1; j<theList.size(); j++){
				//if next element is less than this one, set to min value
				if(comp.compare(theList.get(j), theList.get(minIndex)) < 0){
					minIndex = j;
				}
			}
			//when min is determined, set it to the lowest position available
			swap2(theList, minIndex, i);
		}
	...
	**/
	...
	//overload with comparator
	 public static <T> void mergeSort(List<T> theList,Comparator<T> comp) {
		if(!theList.isEmpty()){
			recursiveMergeSortHelper(theList,0,theList.size(),comp);
		}
    }
	...
	**/
	...
	//overload with comparator
	private static <T> void recursiveMergeSortHelper(List<T> theList, int first, int last, Comparator<T> comp) {
		...
		//recursive calls
		recursiveMergeSortHelper(left,first,mid,comp);
		recursiveMergeSortHelper(right,mid,last,comp);
		//merge lists
		merge(theList,left,right,comp);
	}
	...
	**/
	...
	//overload for Comparator
	private static <T> void merge(List<T> theList, List<T> left, List<T> right, Comparator<T> comp){
		//hold indexes for left, right, and merged
		int i = 0;
		int j = 0;
		int count = 0;
	...
	//comparator
	public static <T> int indexOf(T searchItem, List<T> theList,Comparator<T> comp) {
        return recursiveBinarySearcher(searchItem, theList, 0, theList.size()-1,comp);
    }
	...
	**/
	...
	//overload Comparator 
	private static <T> int recursiveBinarySearcher(T searchItem, List<T> theList, int first, int last, Comparator<T> comp) {
		...
		if(first <= middle && last >= middle){
		//if we found the element, return the index
			if(comp.compare(searchItem, theList.get(middle)) == 0){
				return middle;
			}
			//if lower than middle, recursive with lower half of list
			else if(comp.compare(searchItem, theList.get(middle)) < 0){
				int newLast = middle-1;
				return recursiveBinarySearcher(searchItem, theList, first, newLast,comp);
			}
			//if higher than middle, recursive with higher half of list
			else{
				int newFirst = middle+1;
				return recursiveBinarySearcher(searchItem, theList, newFirst, last,comp);
			}
		}
		//If value is not found, return -1
		else{
