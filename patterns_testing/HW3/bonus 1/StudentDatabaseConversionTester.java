...
public class StudentDatabaseConversionTester{
	...
	/*
	*NEW TESTERS:
	*Formatted to formatted reads in a CSV file, writes to another CSV file,
	*and closes the file. The newly written file is passed through the
	*StudentDatabaseIO and the two are compared for equality
	*/
	
	@Test
    public void testFormattedToFormatted() throws Exception{
        StudentDatabaseCSVFileReader.openFile("studentJUnitTestData.csv");
        StudentDatabase db = StudentDatabaseCSVFileReader.readData();
        StudentDatabaseCSVFileReader.closeFile();
 
        StudentDatabaseCSVFileWriter.openFile("studentJUnitTestData_out15.csv");
        StudentDatabaseCSVFileWriter.writeData(db);
        StudentDatabaseCSVFileWriter.closeFile();
		
        String infile = "studentJUnitTestData_out15.csv";
        String outfile = "studentJUnitTestData16.csv";
        StudentDatabaseIO.formattedToFormatted(infile,outfile);
        ...
        // read it back in as db2
        StudentDatabaseCSVFileReader.openFile("studentJUnitTestData16.csv");
        StudentDatabase db2 = StudentDatabaseCSVFileReader.readData();
        StudentDatabaseCSVFileReader.closeFile();
        // check that they are the same
        assertEquals(db.getNumStudents(), db2.getNumStudents());
	...
	/*
	*Serialized to serialized reads in a ser file, writes to another ser 
	*file, and closes the file. The newly written file is passed through the
	*StudentDatabaseIO and the two are compared for equality
	*/
	
	@Test
    public void testSerializedToSerialized() throws Exception{
        StudentDatabaseSerializedFileReader.openFile("studentJUnitTestData_out2.ser");
        StudentDatabase db = StudentDatabaseSerializedFileReader.readData();
        StudentDatabaseSerializedFileReader.closeFile();
 
        StudentDatabaseSerializedFileWriter.openFile("studentJUnitTestData_out20.ser");
        StudentDatabaseSerializedFileWriter.writeData(db);
        StudentDatabaseSerializedFileWriter.closeFile();

        String infile = "studentJUnitTestData_out20.ser";
        String outfile = "studentJUnitTestData21.ser";
        StudentDatabaseIO.serializedToSerialized(infile,outfile);

        // sort to compare
        db.sortByGPA();

        // read it back in as db2
        StudentDatabaseSerializedFileReader.openFile("studentJUnitTestData21.ser");
        StudentDatabase db2 = StudentDatabaseSerializedFileReader.readData();
        StudentDatabaseSerializedFileReader.closeFile();
        // check that they are the same
        assertEquals(db.getNumStudents(), db2.getNumStudents());
